...
x0     = 2.0
y0     = 2.2
c_xmin = round((x0-r0)/dx)
c_xmax = round((x0+r0)/dx)
c_ymin = round((y0-r0)/dy)
c_ymax = round((y0+r0)/dy)
...
while (t < t_max):
    ...
    v[c_xmin, c_ymin+1:c_ymax+1] =-v[c_xmin-1, c_ymin+1:c_ymax+1]

    # Right obstacle
    u[c_xmax+1, c_ymin:c_ymax+1]   = 0.0
    v[c_xmax,   c_ymin+1:c_ymax+1] =-v[c_xmax+1, c_ymin+1:c_ymax+1]

    # Top obstacle
    u[c_xmin+1:c_xmax+1, c_ymax]  =-u[c_xmin+1:c_xmax+1, c_ymax+1 ]
    v[c_xmin:c_xmax+1:c_ymax+1]   = 0.0

    # Bottom obstacle
    u[c_xmin+1:c_xmax+1, c_ymin]  =-u[c_xmin+1:c_xmax+1, c_ymin-1 ]
    v[c_xmin:c_xmax+1:c_ymin+1]   = 0.0
